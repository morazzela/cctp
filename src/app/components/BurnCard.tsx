...
export default function BurnCard() {
  ...
  const exceedsBurnAllowance = useMemo(() => {
    const limit = burnLimits[srcChain.domain][isV1 ? 1 : 2];
    return limit > 0n && amount > burnLimits[srcChain.domain][isV1 ? 1 : 2];
  ...
  const spender = useMemo(() => {
    return (
      isV1 ? srcChain.tokenMessengerV1 : srcChain.tokenMessengerV2
    ) as Address;
  }, [isV1, srcChain]);
  ...
  useEffect(() => {
    if (dstChain.namespace === srcChain.namespace) {
